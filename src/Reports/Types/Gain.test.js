...
describe('Reports', function() {
	describe('Gain report', function() {
		...
		it('should ignore all sells on tranches that have been generated by non-tax relevant sources', async function() {
			...
			// create three independent buy transactions in a side depot 
			const depositTimestamp = now.minus(Duration.fromObject({years: 0.5}));
			const interestTimestamp = now.minus(Duration.fromObject({years: 0.25}));
			...
			expect(reports.Gain.entries[0].buyRate).to.be.closeTo(2629.97, 0.01);
			expect(reports.Gain.entries[0].gain).to.be.closeTo(685, 0.1);
			...
			expect(reports.Gain.entries[1].buyRate).to.be.closeTo(2629.97, 0.01);
			expect(reports.Gain.entries[1].gain).to.be.closeTo(735, 0.1);
		});
	});
});
